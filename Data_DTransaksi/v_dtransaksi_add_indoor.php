        ...
        <div class="card-body table-responsive">
            <div class="row content-header border shadow-sm">
                <div class="col-md-6">
                    ...
                    ?>
                    <h4>Transaksi Indoor - <?= $dnm['nama_customer']; ?></h4>
                </div>
                <div class="col-md-6">
            ...
            <hr>
            <!-- Form add customer -->
            <form class="form-horizontal col-md-6" action="<?= $_SESSIONCF\['level'] == "CS" ? 'Code_DTransaksi' : '../Data_Online/Code_Online' ?>/c_dtransaksi_add_indoor.php" method="post">
                <div class="form-group row">
                    <?php
                        $str     = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
                        $rndm    = str_shuffle($str);
                        $sub     = date('dmy').substr($rndm,0,5);
                        $id_dtransaksi = "[;TI".$sub;
                    ?>
                    ...
                    <div class="col-md-8">
                        <div class="input-group">
                            <select class="form-control select2 bahan" name="jenis_bahan">
                                ...
                                <?php
                                    $qjb    = mysqli_query($koneksi,"SELECT * FROM jenis_bahan WHERE ket_bahan = '1' AND status_bahan = 'Tersedia'");
...
<script>
    $(document).ready(function() {
        ...
        $(".bahan").change( function(){
            <?php
                $q11 = mysqli_query($koneksi,"SELECT * FROM jenis_bahan WHERE ket_bahan = '1' AND status_bahan = 'Tersedia'");
                while($d11=mysqli_fetch_array($q11)){ 
            ?>
                if($(".bahan").val()=="<?= $d11['nama_bahan']; ?>"){ 
                
                    $(".total").val($(".jumlah").val() * <?= $d11['harga_bahan']; ?>);
